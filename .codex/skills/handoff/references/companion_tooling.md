...
- **Implicitly**, when the request matches the description ("hand this off", "write me a handoff").
- **Explicitly**, with the next-session focus as the argument: `$handoff ship the .codex skills batch`.

Whatever the user says after the skill name is the next-session focus; pass it straight to `--next-focus`.

The hard rule survives the port: **if you find yourself typing content out of a PRD, plan, ADR or issue, stop and replace it with a reference.**
...
- **OpenAI — Codex documentation** (https://developers.openai.com/codex/) — session and skill model this port targets
