...
- **Implicitly**, when the request matches the description ("be brief", "less tokens", "caveman mode").
- **Explicitly**, by naming it: `$caveman`.

Codex does not carry a skill across turns unless it is re-mentioned, which works against the persistence rule in `SKILL.md`. Two practical consequences:

1. Announce the mode on the first caveman turn, so the user knows it is meant to persist and can see when it has lapsed.
2. If the mode does lapse, `$caveman` re-arms it in one word — cheaper than re-explaining.
...
- **OpenAI — Prompt engineering guide** (https://platform.openai.com/docs/guides/prompt-engineering) — token-conscious prompting
