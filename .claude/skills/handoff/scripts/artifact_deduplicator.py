...
def _match_header_in_line(line: str, line_no: int, header: str, kind: str, suggestion: str) -> dict[str, Any] | None:
    if header in line.lower() and ("#" in line or ":" in line):
        return _make_finding(line_no, kind, header, line.strip(), suggestion)
    return None


def _match_field_in_line(line: str, line_no: int, field: str, kind: str, suggestion: str) -> dict[str, Any] | None:
